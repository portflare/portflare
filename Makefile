...
LDFLAGS := -s -w \
	-X github.com/portflare/portflare/internal/buildinfo.Version=$(VERSION) \
	-X github.com/portflare/portflare/internal/buildinfo.Commit=$(COMMIT) \
	-X github.com/portflare/portflare/internal/buildinfo.Date=$(BUILD_DATE)

SERVER_PKG := ./cmd/reverse-server
CLIENT_PKG := ./cmd/reverse-client

.PHONY: build build-server build-client release clean
...
release:
	...
	for platform in $(PLATFORMS); do \
		...
		cp "$(BIN_DIR)/$${client_bin}" "$(RELEASE_DIR)/$${archive_base}/"; \
